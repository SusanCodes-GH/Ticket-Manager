...
export const getTickets = async (req, res) => {
  try {
    if (req.user.role === 'team') {
      const tickets = await ticketService.getTicketsForUser(req.user.uid);
...
export const createTicket = async (req, res) => {
  try {
    ...
    const ticket = await ticketService.createTicket(req.body, req.user.uid);
...
export const updateTicket = async (req, res) => {
  try {
    ...
    if (req.user.role === 'team') {
      ...
      const ticket = await ticketService.updateTicket(id, allowedFields, req.user.uid);
      return successResponse(res, ticket, 'Ticket updated successfully');
    }

    const ticket = await ticketService.updateTicket(id, req.body, req.user.uid);
    return successResponse(res, ticket, 'Ticket updated successfully');
  } catch (error) {
    return errorResponse(res, error.message, 400);
  }
...
export const assignTicket = async (req, res) => {
  try {
    ...
    const ticket = await ticketService.assignTicket(id, assignedTo, priority, req.user.uid);
...
export const updateStatus = async (req, res) => {
  try {
    ...
    const ticket = await ticketService.updateStatus(id, status, req.user.uid);
...
export const addComment = async (req, res) => {
  try {
    ...
    const result = await ticketService.addComment(id, req.user.uid, comment);
